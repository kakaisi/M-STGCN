...
def load_ST_file(dataset, highly_genes, k, radius):
    ...
    # pfea = pd.read_csv(kegg_path, index_col=0)

    pfea = np.load(image_path)
    # pfea = pfea.values.astype('float32')
    pfea = pfea.astype('float32')
    padj = features_construct_graph(pfea, k=k)

    adata.X = np.load(kegg_path)
